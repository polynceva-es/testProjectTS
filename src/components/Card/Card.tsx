...
return (
    <article className={cardClassName}>
        <img src={card.avatar} alt="photo" className='card__avatar' onClick={() => navigate(`/users/${card.id}`)}/>
          <h1>{card.first_name}</h1>
          <p>{card.last_name}</p>
          <p>{card.email}</p>
          <button className={buttonLikeClassName} onClick={()=> dispatch(handleLike(card.id))}/>
          <button className='card__button-delete' onClick={()=> dispatch(handleDelete(card.id))}/>
    </article>
)
}